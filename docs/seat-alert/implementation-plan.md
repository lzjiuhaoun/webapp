...
字段不固定，按日志源动态自配。运算符包含：equals（等于）、notEquals（不等于）、gte（大于等于）、lte（小于等于）、contains（包含）、notContains（不包含）、crossCamp（跨阵营）。
...
| 运算符 | 说明 | 适用场景 |
|--------|------|----------|
| equals | 字段值精确匹配 | operatorName、operatorAccount 等精确字段 |
| notEquals | 字段值取反匹配 | 排除特定值 |
| gte | 数值比较 >= | loginTime >= 800 |
| lte | 数值比较 <= | loginTime <= 1900 |
| contains | 模糊子串匹配 | fileName 包含"机密"等敏感词 |
| crossCamp | 参与方跨阵营检测 | 发送方与接收方不在同一阵营组 |
...
- **工作时间示例**：条件1=`loginTime gte 800`，条件2=`loginTime lte 1900`，关联方式=与
- **非工作时间示例**：条件1=`loginTime lt 800`，条件2=`loginTime gt 1900`，关联方式=或（跨午夜场景可建单规则用圆环逻辑）
...
| conditions | TEXT | JSON 字符串: `[{"field":"fileName","operator":"contains","value":"机密"},...]` |
